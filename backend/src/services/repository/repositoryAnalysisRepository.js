...
export async function saveRepositoryAnalysis({
  ...
  if (!githubUserId) throw new Error("GitHub user ID is required.");
  if (!repositoryUrl) throw new Error("Repository URL is required.");
  if (!profile) throw new Error("Repository profile is required.");

  const repositoryInfo = parseGitHubRepository(repositoryUrl);
  if (!repositoryInfo) throw new Error("Invalid GitHub repository URL.");
  ...
  return await sql.begin(async (tx) => {
    ...
    const infrastructure = [
      { type: "docker", data: profile.infrastructure?.docker },
      { type: "terraform", data: profile.infrastructure?.terraform },
      { type: "kubernetes", data: profile.infrastructure?.kubernetes },
      { type: "ci_cd", data: profile.ci_cd?.githubActions },
    ];
    ...
    for (const item of infrastructure) {
      ...
      const detectedFiles = detection.files || detection.workflows || [];
...
export async function getLatestRepositoryAnalysis(githubUserId) {
  ...
  const languages = await sql`
    SELECT language, file_count FROM public.scan_languages
    WHERE scan_id = ${scan.scan_id}
    ORDER BY file_count DESC;
  `;

  const applications = await sql`
    SELECT name, directory, type, framework, package_manager, runtime
  ...
  const packages = await sql`
    SELECT id, directory, package_file, name, version, package_manager, scripts
  ...
  const dependencies = await sql`
    SELECT pd.package_id, pd.dependency_name, pd.dependency_type
    FROM public.package_dependencies pd
    INNER JOIN public.scan_packages sp ON sp.id = pd.package_id
    WHERE sp.scan_id = ${scan.scan_id}
    ORDER BY pd.dependency_name;
  `;

  const databases = await sql`
    SELECT name, directory, confidence FROM public.scan_databases
    WHERE scan_id = ${scan.scan_id}
    ORDER BY name;
  `;

  const infrastructureRows = await sql`
    SELECT id, infrastructure_type, detected FROM public.scan_infrastructure
    WHERE scan_id = ${scan.scan_id};
  `;

  const infrastructureFiles = await sql`
    SELECT sif.infrastructure_id, sif.file_path
    FROM public.infrastructure_files sif
    INNER JOIN public.scan_infrastructure si ON si.id = sif.infrastructure_id
    WHERE si.scan_id = ${scan.scan_id};
  `;

  const evidence = await sql`
    SELECT evidence_type AS type, technology, directory, confidence, source, reason
  ...
  const dependencyProfile = packages.map((pkg) => {
    const pkgDependencies = dependencies.filter((dependency) => dependency.package_id === pkg.id);
    return {
      ...
      production: pkgDependencies.filter((dependency) => dependency.dependency_type === "production").map((dependency) => dependency.dependency_name),
      development: pkgDependencies.filter((dependency) => dependency.dependency_type === "development").map((dependency) => dependency.dependency_name),
      scripts: pkg.scripts || {},
    };
  });

  const infrastructure = {
    docker: { detected: false, files: [], evidence: [] },
    terraform: { detected: false, files: [], evidence: [] },
    kubernetes: { detected: false, files: [], evidence: [] },
  };

  let githubActions = { detected: false, workflows: [], evidence: [] };

  for (const row of infrastructureRows) {
    const files = infrastructureFiles.filter((file) => file.infrastructure_id === row.id).map((file) => file.file_path);
    const infrastructureEvidence = evidence.filter((item) => (row.infrastructure_type === "ci_cd" ? item.type === "ci_cd" : item.type === row.infrastructure_type));
    const value = { detected: row.detected, files, evidence: infrastructureEvidence };

    if (row.infrastructure_type === "docker") infrastructure.docker = value;
    if (row.infrastructure_type === "terraform") infrastructure.terraform = value;
    if (row.infrastructure_type === "kubernetes") infrastructure.kubernetes = value;
    if (row.infrastructure_type === "ci_cd") githubActions = { detected: row.detected, workflows: files, evidence: infrastructureEvidence };
  }

  return {
    profile: {
      repository: { url: scan.repository_url },
      statistics: { totalFiles: scan.total_files, totalApplications: scan.total_applications },
      languages: {
        primary: scan.primary_language,
        detected: languages.map((language) => ({ language: language.language, fileCount: language.file_count })),
      },
      applications,
...
function parseGitHubRepository(repositoryUrl) {
  try {
    ...
    return { owner, name, fullName: `${owner}/${name}`, branch: "main" };
  } catch {
    return null;
  }
}